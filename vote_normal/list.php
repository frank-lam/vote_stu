...
//  	}
    ...
    $pdo = connectDb();
    $sql = "select * from vote_person_info order by vote_time desc";
    $arr = $pdo->query($sql);
    ...
    <div class="container">
        <table class="table table-condensed table-hover table-bordered text-center">
            <h4 class="text-center">投票代表信息一览</h4>
            ...
            foreach ($arr as $row) {
                ...
                echo "<tr>";
	                echo "<td>{$id}</td>";
	                echo "<td>{$row['vote_stu']}</td>";

	
	                echo "<td>{$row['vote_name']} </td>";
	                echo "<td>{$row['vote_mark']}</td>";
	                echo "<td>{$row['vote_mobile']}</td>";
					echo "<td>{$row['vote_time']}</td>";
	                echo "<td>
						<a href='javascript:doDel({$row['vote_id']})'>删除</a>
						<a href='edit.php?id={$row['vote_id']}'>修改</a>
					</td>";
                echo "<tr/>";
            }
            ?>
